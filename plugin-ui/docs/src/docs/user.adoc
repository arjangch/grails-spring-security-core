...
This allows you to temporarily assume the identity of another User (see https://apache.github.io/grails-spring-security/7.0.x/core-plugin/guide/index.html#switchUser#switchUser[the Grails Spring Security Core Plugin documentation] for more information about switch-user). The "Logged in as ..." information in the top right of the screen will change to show that you're running as another User and provide a link to switch back. The role name `ROLE_SWITCH_USER` is the default but you can change the value with the `grails.plugin.springsecurity.ui.switchUserRoleName` setting in application.groovy.
